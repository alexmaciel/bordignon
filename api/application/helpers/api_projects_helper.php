 ...
 * @return string
 */
function project_image_url($project_id, $type = '')
{
    $url  = '';
    $CI   = &get_instance();
    ...
    if (!$path) {
        ...
        //$pic = $CI->projects_model->get_picture($project_id);
        $CI->db->order_by('order', 'asc');

        $CI->db->where('project_id', $project_id);
        $pic = $CI->db->get(db_prefix() . 'projects_pictures')->row();
