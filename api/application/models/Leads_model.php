...
class Leads_model extends Api_Model
{
    ...
    public function get_status($id = '', $where = [])
    {
        ...
        $statuses = $this->api_object_cache->get('leads-all-statuses');

        if (!$statuses) {
            $this->db->order_by('statusorder', 'asc');

            $statuses = $this->db->get(db_prefix() . 'leads_status')->result_array();
            $this->api_object_cache->add('leads-all-statuses', $statuses);
        }
     ...
     */
    public function log_lead_activity($id, $staffid, $description, $integration = false, $additional_data = '')
    {
        $log = [
