...
class Clients_model extends Api_Model
{
    ...
    public function getTable()
    {
        ...
        $this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . 'clients')) . ',' . get_sql_select_client_company() . ', CONCAT(firstname, " ", lastname) as fullname, email' . ',' . db_prefix() . 'clients.datecreated as date' );
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false)
    {
        ...
        if (isset($data['password'])) {
            ...
            $data['password']     = app_hash_password(isset($data['password']) ? $data['password'] : time());
        }  
        ...
        if ($contact_id) {
            ...
            if ($send_set_password_email) {
                ...
                $this->authentication_model->set_password_email($data['email'], true);
            }
    ...
    public function delete_picture($userid)
    {
        ...
        if ($file) {
            $path     = get_upload_path_by_type('clients') . $userid . '/';
    ...
    public function send_verification_email($id)
    {
        ...
        $merge_fields = array_merge($merge_fields, $api_merge_fields->get_client_contact_merge_fields($contact->id));
